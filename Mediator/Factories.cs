...
namespace Mediator
{
    public partial class Factories : Form
    {
        ...
        void clear_all()
        {
            try
            {
                // TODO: This line of code loads data into the 'MediatorDataSet.Factories' table. You can move, or remove it, as needed.
                this.factoriesTableAdapter.Fill(this.MediatorDataSet.Factories);

                factory_nameTextBox.Clear();
                factory_detailsTextBox.Clear();
        ...
        private void Search_textBox_TextChanged(object sender, EventArgs e)
        {
            try
            {
                this.factoriesTableAdapter.Search(this.MediatorDataSet.Factories, Search_textBox.Text);
            }
            catch (Exception errortext)
            {
                LoginInfo.log(" حدث خطأ فى " + this.Text + " " + this.Name.ToString() + " " + errortext.ToString());
        ...
        private void add_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(factory_nameTextBox.Text) && !string.IsNullOrWhiteSpace(factory_nameTextBox.Text))
                {

                this.factoriesTableAdapter.Insert(factory_nameTextBox.Text, factory_detailsTextBox.Text);
                ...
                if (!string.IsNullOrEmpty(factory_nameTextBox.Text) && !string.IsNullOrWhiteSpace(factory_nameTextBox.Text))
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (product != null)
                        {
                            ...
                            this.factoriesTableAdapter.Update(factory_nameTextBox.Text,  factory_detailsTextBox.Text, id);
        ...
        private void delete_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        int id = int.Parse(product.Cells[0].Value.ToString());
                        this.factoriesTableAdapter.Delete(id);
