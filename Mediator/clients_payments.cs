...
namespace Mediator
{
    public partial class clients_payments : Form
    {
        public clients_payments()
        {
            InitializeComponent();
        }
        ...
        void clear_all()
        {
            try
            {
                ...
                this.clients_PaymentsTableAdapter.Fill(this.MediatorDataSet.Clients_Payments);

                payment_detailsTextBox.Clear();
                payment_dateDateTimePicker.ResetText();
                payment_amountNumericUpDown.ResetText();
                payment_client_idComboBox.ResetText();
        ...
        private void Search_textBox_TextChanged(object sender, EventArgs e)
        {
            try
            {
                this.clients_PaymentsTableAdapter.Search(this.MediatorDataSet.Clients_Payments, Search_textBox.Text);
            }
            catch (Exception errortext)
            {
                LoginInfo.log(" حدث خطأ فى " + this.Text + " " + this.Name.ToString() + " " + errortext.ToString());
        ...
        private void add_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.clients_PaymentsTableAdapter.Insert(int.Parse(LoginInfo.id), (int)payment_client_idComboBox.SelectedValue,payment_amountNumericUpDown.Value, payment_dateDateTimePicker.Value,payment_detailsTextBox.Text);
                ...
                if (payment_amountNumericUpDown.Value != 0)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (product != null)
                        {
                            ...
                            this.clients_PaymentsTableAdapter.Update(int.Parse(LoginInfo.id), (int)payment_client_idComboBox.SelectedValue, payment_amountNumericUpDown.Value, payment_dateDateTimePicker.Value, payment_detailsTextBox.Text, id);
        ...
        private void delete_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        int id = int.Parse(product.Cells[0].Value.ToString());
                        this.clients_PaymentsTableAdapter.Delete(id);
