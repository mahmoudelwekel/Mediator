...
namespace Mediator
{
    public partial class clients : Form
    {
        ...
        void clear_all()
        {
            try
            {
                // TODO: This line of code loads data into the 'MediatorDataSet.clients' table. You can move, or remove it, as needed.
                this.ClientsTableAdapter.Fill(this.MediatorDataSet.Clients);

                client_nameTextBox.Clear();
                client_phoneTextBox.Clear();
                client_detailsTextBox.Clear();
                client_profitNumericUpDown.ResetText();
        ...
        private void Search_textBox_TextChanged(object sender, EventArgs e)
        {
            try
            {
                this.ClientsTableAdapter.Search(this.MediatorDataSet.Clients, Search_textBox.Text);
            }
            catch (Exception errortext)
            {
                LoginInfo.log(" حدث خطأ فى " + this.Text + " " + this.Name.ToString() + " " + errortext.ToString());
        ...
        private void add_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(client_nameTextBox.Text) && !string.IsNullOrWhiteSpace(client_nameTextBox.Text))
                {

                this.ClientsTableAdapter.Insert(client_nameTextBox.Text, client_profitNumericUpDown.Value, client_phoneTextBox.Text, client_detailsTextBox.Text);
                ...
                if (!string.IsNullOrEmpty(client_nameTextBox.Text) && !string.IsNullOrWhiteSpace(client_nameTextBox.Text))
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (product != null)
                        {
                            ...
                            this.ClientsTableAdapter.Update(client_nameTextBox.Text, client_profitNumericUpDown.Value, client_phoneTextBox.Text, client_detailsTextBox.Text, id);
        ...
        private void delete_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    {
                        int id = int.Parse(product.Cells[0].Value.ToString());
                        this.ClientsTableAdapter.Delete(id);
